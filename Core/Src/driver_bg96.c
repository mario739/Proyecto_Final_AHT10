...
em_bg96_error_handling get_status_modem(st_bg96_config*obj)
{
    ...
    ft_resp=obj->send_data_device(CMD_BG96_STATUS_MODEM,RS_BG96_OK,buffer_receive,10);
    return ft_resp;
}
...
em_bg96_error_handling publish_message(st_bg96_config *obj,st_config_parameters_mqtt *obj_mqtt,char *topic,char *data)
{
    em_bg96_error_handling ft_resp=FT_BG96_ERROR;
    char buffer_receive[70]={0};
    char buffer_receive2[70]={0};
    char buffer_receive3[30]={0};
    char cmd[50]={0};
    ...
    if (FT_BG96_OK==ft_resp)
    {
        ft_resp=obj->send_data_device(buffer_data,RS_BG96_CERO,buffer_receive2,15000);

       //if (FT_BG96_OK==ft_resp)
        //{
           // ft_resp=obj->send_data_device("\x1A\r",RS_BG96_CERO,buffer_receive3,15000);
        //}
    }
    return ft_resp;
}
